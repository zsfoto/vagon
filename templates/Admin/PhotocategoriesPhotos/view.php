						...
						<div class="card mb-3">
							...
							<div class="card-header">
								<div class="float-start">
									<h3><i id="card-icon" class="fa fa-eye fa-spin"></i> <?= __('View') ?>: <?= __('Photocategories Photo') ?></h3>
...
<?php if($config['show_id']){ ?>
											<div class="row"><!-- 3. -->
												...
												<div class="col-sm-10 p-1">
													<?= $photocategoriesPhoto->id ?><!-- 0.a -->
												</div>
											</div>
<?php } ?>
											<div class="row"><!-- 1. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Photocategory') ?>:</label>
												<div class="col-sm-10 p-1 link">
													<?= $photocategoriesPhoto->hasValue('photocategory') ? $this->Html->link($photocategoriesPhoto->photocategory->name, ['controller' => 'Photocategories', 'action' => 'view', $photocategoriesPhoto->photocategory->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span>
												</div>
											</div>
											<div class="row"><!-- 1. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Photo') ?>:</label>
												<div class="col-sm-10 p-1 link">
													<?= $photocategoriesPhoto->hasValue('photo') ? $this->Html->link($photocategoriesPhoto->photo->name, ['controller' => 'Photos', 'action' => 'view', $photocategoriesPhoto->photo->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span>
												</div>
											</div>
