...
class EmailtemplatesController extends AppController
{
    ...
    {
		$this->set('title', __('Browse the') . ': ' . __('Emailtemplates'));
		...
		$this->paginate['Emailtemplates']['order'] = [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Emailtemplates']['limit'] = $this->config['paginate_limit'];
		...
		//debug(empty($emailtemplates->toArray()));
		//die();
		
		if(empty($emailtemplates->toArray())){
    ...
    {
		$this->set('title', __('View the') . ': ' . __('emailtemplate') . ' ' . __('record'));
        $emailtemplate = $this->Emailtemplates->get($id, contain: []);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $emailtemplate->name;
        $this->set(compact('emailtemplate', 'id', 'name'));
    }
    ...
    public function add()
    {
		$this->set('title', __('Add new') . ': ' . __('emailtemplate') . ' ' . __('record'));
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Emailtemplates->save($emailtemplate)) {
                $this->Flash->success(__('The emailtemplate has been saved.'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $emailtemplate->id);

                return $this->redirect(['action' => 'index']);
            }
            $this->Flash->error(__('The emailtemplate could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
        }
    ...
    {
		$this->set('title', __('Edit the') . ': ' . __('emailtemplate') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Emailtemplates->delete($emailtemplate)) {
            ...
            $this->Flash->success(__('The emailtemplate has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            $this->Flash->error(__('The emailtemplate could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
        }
